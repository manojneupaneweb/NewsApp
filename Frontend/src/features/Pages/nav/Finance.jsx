...
const Finance = () => {
  ...
  useEffect(() => {
    const getFinanceNews = async () => {
      try {
        const data = await fetchPostsByCategory("finance", currentPage, postsPerPage);

        // Since the response is an array of posts, directly set it
        setPosts(data); // Directly set the posts data

        // You can calculate `totalPosts` based on the number of posts you have, 
        // or handle this in a separate API call to fetch the total number of posts.
        setTotalPosts(data.length); // Assuming that 'data' is an array of posts
      } catch (error) {
  ...
  return (
    <section className="py-10 px-5 md:px-20 bg-gray-100">
      <h1 className="text-green-500 font-bold text-3xl mb-6 flex items-center gap-3">
      ...
      {posts.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {posts.map((post) => (
            ...
            >
              ...
              <h2 className="text-green-500 font-semibold text-xl mt-4 hover:text-green-400 cursor-pointer">
        ...
        <p className="text-center text-gray-500 text-lg mt-10">हाल वित्तीय समाचार उपलब्ध छैन।</p>
      )}
